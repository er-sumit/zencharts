	...
	private _updateMarks(firstValue: number, span: number, high: number, low: number, minCoord: number, maxCoord: number): void {
		...
		for (let logical = high - mod; logical > low; logical -= span) {
			...
			updateObjectPool(
				...
				targetIndex,
				(v) => ({
					coord: v.coord as Coordinate,
					label: priceScale.formatLogical(v.logical),
					logical: v.logical,
				}),
				(item, v) => {
					item.coord = v.coord as Coordinate;
					item.label = priceScale.formatLogical(v.logical);
					item.logical = v.logical;
				},
				{ coord, logical }
			);
