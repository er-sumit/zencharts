...
import { createPreconfiguredCanvasBinding, drawWidgetBitmap, releaseCanvas } from './canvas-utils';
...
export class PriceAxisWidget implements IDestroyable {
	...
	public constructor(pane: PaneWidget, options: Readonly<ChartOptionsInternalBase>, rendererOptionsProvider: PriceAxisRendererOptionsProvider, side: PriceAxisWidgetSide) {
		...
		this._canvasBinding = createPreconfiguredCanvasBinding(this._cell, this._canvasSuggestedBitmapSizeChangedHandler, '1');
		this._topCanvasBinding = createPreconfiguredCanvasBinding(this._cell, this._topCanvasSuggestedBitmapSizeChangedHandler, '2');
	...
	public drawBitmap(ctx: CanvasRenderingContext2D, x: number, y: number, addTopLayer?: boolean): void {
		drawWidgetBitmap(this._canvasBinding, ctx, x, y, addTopLayer ? this._topCanvasBinding : undefined);
	}
