...
function beforeInteractions(container) {
	...
	allSeriesChart = ZenCharts.createChart(allSeriesContainer, {
	...
	pointSeries = allSeriesChart.addSeries(ZenCharts.LineSeries, {
		hitTestTolerance: 8,
		lineWidth: 4,
	...
	priceLineSeries = allSeriesChart.addSeries(ZenCharts.LineSeries, {
		lastValueVisible: false,
	}, priceLinePane.paneIndex());

	markerSeries = allSeriesChart.addSeries(ZenCharts.LineSeries, {
		lineWidth: 3,
		pointMarkersVisible: false,
	}, markerPane.paneIndex());

	areaSeries = allSeriesChart.addSeries(ZenCharts.AreaSeries, {
		lineWidth: 4,
	}, areaPane.paneIndex());

	baselineSeries = allSeriesChart.addSeries(ZenCharts.BaselineSeries, {
		lineWidth: 4,
	}, baselinePane.paneIndex());

	barSeries = allSeriesChart.addSeries(ZenCharts.BarSeries, {
		hitTestTolerance: 8,
	}, barPane.paneIndex());

	candlestickSeries = allSeriesChart.addSeries(ZenCharts.CandlestickSeries, {}, candlePane.paneIndex());

	histogramSeries = allSeriesChart.addSeries(ZenCharts.HistogramSeries, {
		base: 0,
		hitTestTolerance: 8,
	}, histogramPane.paneIndex());

	steppedSeries = allSeriesChart.addSeries(ZenCharts.LineSeries, {
		lineType: ZenCharts.LineType.WithSteps,
		lineWidth: 5,
	}, steppedPane.paneIndex());

	curvedSeries = allSeriesChart.addSeries(ZenCharts.LineSeries, {
		lineType: ZenCharts.LineType.Curved,
		lineWidth: 10,
	}, curvedPane.paneIndex());

	singlePointSeries = allSeriesChart.addSeries(ZenCharts.LineSeries, {
		lineWidth: 6,
		pointMarkersVisible: false,
	...
	markerPrimitives = ZenCharts.createSeriesMarkers(markerSeries, [
		{
	...
	overlapChart = ZenCharts.createChart(overlapContainer, {
		hoveredSeriesOnTop: false,
		rightPriceScale: { visible: false },
	});

	const backSeries = overlapChart.addSeries(ZenCharts.LineSeries, {
		color: '#2563eb',
		lineWidth: 5,
		pointMarkersVisible: true,
		pointMarkersRadius: 4,
	});
	const frontSeries = overlapChart.addSeries(ZenCharts.LineSeries, {
		color: '#dc2626',
		lineWidth: 3,
		pointMarkersVisible: true,
		pointMarkersRadius: 4,
