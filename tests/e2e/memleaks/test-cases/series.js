...
export const scenario = {
	...
	setup: async function(page) {
		...
		await page.evaluate(() => {
			window.chart = ZenCharts.createChart(
	...
	action: async function(page) {
		await page.evaluate(() => {
			...
			if (window.chart) {
				window.lineSeries = window.chart.addSeries(ZenCharts.LineSeries);
				window.lineSeries.setData(generateData());
				window.areaSeries = window.chart.addSeries(ZenCharts.AreaSeries);
				window.areaSeries.setData(generateData());
				window.baselineSeries = window.chart.addSeries(ZenCharts.BaselineSeries);
				window.baselineSeries.setData(generateData());
				window.histogramSeries = window.chart.addSeries(ZenCharts.HistogramSeries);
				window.histogramSeries.setData(generateData());

				window.barSeries = window.chart.addSeries(ZenCharts.BarSeries);
				window.barSeries.setData(generateBars());
				window.candlestickSeries = window.chart.addSeries(ZenCharts.CandlestickSeries);
