...
function convertTime(t) {
	if (ZenCharts.isUTCTimestamp(t)) {
		return t * 1000;
	}
	if (ZenCharts.isBusinessDay(t)) {
...
const DefaultHorzScaleBehavior	= ZenCharts.defaultHorzScaleBehavior();
...
function runTestCase(container) {
	const chart = window.chart = ZenCharts.createChartEx(container, new CustomBehavior(), {
		width: 800,
		height: 400,
	...
	chart.timeScale().applyOptions({
		tickMarkFormatter: (time, tickMarkType) => {
			...
			if (
				...
				if (tickMarkType === ZenCharts.TickMarkType.DayOfMonth) {
					if (
