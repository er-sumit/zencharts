...
function runTestCase(container) {
	const chart = window.chart = ZenCharts.createChart(container, { addDefaultPane: false, layout: { attributionLogo: false } });

	// eslint-disable-next-line no-unused-vars
	const pane0 = chart.addPane(true);
	const pane1 = chart.addPane(true);
	const mainSeries = pane1.addSeries(ZenCharts.BarSeries);
	const thirdSeries = chart.addSeries(ZenCharts.BarSeries, {}, 2);
