...
function runTestCase(container) {
	...
	const leftSeries = chart.addSeries(ZenCharts.LineSeries, {
		priceScaleId: 'left',
		color: 'blue'
	});
	const rightSeries = chart.addSeries(ZenCharts.LineSeries, {
		priceScaleId: 'right',
		color: 'red'
	});
	...
	const rightData = [];
	for (let i = 0; i < 100; ++i) {
		const time = 1600000000 + i * 86400;
		// Left series operates on small numbers: 0 to 10
		leftData.push({ time, value: i * 0.1 });
		// Right series operates on large numbers: 1000 to 2000
		rightData.push({ time, value: 1000 + (i * 10) });
	}
	leftSeries.setData(leftData);
	rightSeries.setData(rightData);

	// Trend line on left series: crosses 1 to 9
	const leftTrend = new TrendLine(
		chart, 
		leftSeries, 
		{ time: 1600000000 + 10 * 86400, price: 1 }, 
		{ time: 1600000000 + 90 * 86400, price: 9 },
		{ width: 4, lineColor: 'cyan' }
	);
	leftSeries.attachPrimitive(leftTrend);

	// Trend line on right series: crosses 1100 to 1900
	const rightTrend = new TrendLine(
		chart, 
		rightSeries, 
		{ time: 1600000000 + 10 * 86400, price: 1100 }, 
		{ time: 1600000000 + 90 * 86400, price: 1900 },
		{ width: 4, lineColor: 'orange' }
	);
	rightSeries.attachPrimitive(rightTrend);

	leftSeries.update({ time: 1600000000 + 100 * 86400, value: 10 });
	rightSeries.update({ time: 1600000000 + 100 * 86400, value: 2000 });
